...
exports.InMemorySkillRegistry = void 0;
...
class InMemorySkillRegistry {
    ...
    async register(skill) {
        ...
        const nameConflict = existingSkillsInLayer.find(existing => existing.name.toLowerCase() === skill.name.toLowerCase());
        if (nameConflict) {
    ...
    async search(searchTerm) {
        ...
        return Array.from(this.skills.values()).filter(skill => skill.name.toLowerCase().includes(term) ||
    ...
    async getDependentSkills(skillId) {
        return Array.from(this.skills.values()).filter(skill => skill.dependencies.some(dep => dep.id === skillId));
    }
}
