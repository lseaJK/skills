...
exports.SkillEditorProvider = void 0;
const vscode = __importStar(require("vscode"));
/**
 * Custom editor provider for skill files
 */
class SkillEditorProvider {
    constructor(context, configManager, skillRegistry) {
    ...
    async resolveCustomTextEditor(document, webviewPanel, token) {
        ...
        webviewPanel.webview.onDidReceiveMessage(message => this.handleWebviewMessage(message, document, webviewPanel), undefined, this.context.subscriptions);
    ...
    getWebviewContent(webview, document) {
        const scriptUri = webview.asWebviewUri(vscode.Uri.joinPath(this.context.extensionUri, 'media', 'skillEditor.js'));
        const styleUri = webview.asWebviewUri(vscode.Uri.joinPath(this.context.extensionUri, 'media', 'skillEditor.css'));
...
<body>
     ...
     */
    async validateSkill(skill, webview) {
        try {
     ...
     */
    async previewSkill(skill, webview) {
        try {
     ...
     */
    async saveSkill(skill, document) {
        // Update metadata
        skill.metadata = {
            ...skill.metadata,
     ...
     */
    async updateDocument(skill, document) {
     ...
     */
    async getAutoComplete(context, webview) {
        try {
            const suggestions = [];
            // Provide suggestions based on context
            switch (context.field) {
                case 'category':
                    suggestions.push('general', 'data-processing', 'file-operations', 'network', 'database', 'ui', 'testing', 'deployment', 'monitoring');
                    break;
                case 'tags':
                    suggestions.push('utility', 'automation', 'integration', 'transformation', 'validation', 'security', 'performance', 'debugging');
                    break;
                case 'parameterType':
                    suggestions.push('string', 'number', 'boolean', 'object', 'array');
                    break;
                case 'schemaType':
                    suggestions.push('{"type": "object", "properties": {}}', '{"type": "string"}', '{"type": "number"}', '{"type": "boolean"}', '{"type": "array", "items": {}}');
                    break;
                default:
            ...
            webview.postMessage({
                ...
                suggestions: suggestions.filter(s => s.toLowerCase().includes((context.query || '').toLowerCase())).slice(0, 10)
            });
        }
        catch (error) {
            webview.postMessage({
     ...
     */
    async registerSkill(skill, webview) {
        try {
...
exports.SkillEditorProvider = SkillEditorProvider;
SkillEditorProvider.viewType = 'skillsArchitecture.skillEditor';
//# sourceMappingURL=skillEditorProvider.js.map